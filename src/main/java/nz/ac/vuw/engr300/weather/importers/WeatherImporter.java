...
public class WeatherImporter {
  ...
  private void processWeather(JsonElement weatherInformation) {
    ...
    for (JsonElement content : weatherStuff) {
      ...
      JsonObject weatherDetails = accessContent.get("weather").getAsJsonArray().get(0)
          .getAsJsonObject();
      WeatherConditionData condition = new WeatherConditionData(
          weatherDetails.get("main").getAsString(),
          weatherDetails.get("description").getAsString());

      this.weatherDetails
          .add(new WeatherData(timestamp, speed, angle, temp, pressure, humidity, condition));
    }
  }
  ...
  public WeatherData getWeather(int position) {
    if (position < 0 || position >= this.weatherDetails.size()) {
      throw new IllegalArgumentException(
          "The position " + position + " is not within the valid data range.");
    }
